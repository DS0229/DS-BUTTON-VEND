                ...
                if not isinstance(message.channel, discord.channel.DMChannel):
                    ...
                    if 1 <= amount <= 30:
                        ...
                        for _ in range(amount):
                            code = "DSVEND-" + randomstring.pick(20)
                            codes.append(code)
                ...
                if (search_result != None):
                    if (search_result[2] == 0):
                        if not (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                            ...
                            cur = con.cursor()
                            cur.execute("CREATE TABLE serverinfo (id TEXT, expiredate TEXT, cultureid TEXT, culturepw TEXT, pw TEXT, roleid TEXT, logwebhk TEXT, buylogwebhk TEXT, culture_fee TEXT, bank TEXT, normaloff INTEGER, vipoff INTEGER, vvipoff INTEGER, reselloff INTEGER, color TEXT, chargeban INTEGER, vipautosetting INTEGER, vvipautosetting INTEGER, buyusernamehide TEXT, viproleid INTEGER, vviproleid INTEGER, pushbulletapppassword TEXT, webhookprofile TEXT, webhookname TEXT);")
                            con.commit()
                            first_pw = randomstring.pick(10)
                            cur.execute("INSERT INTO serverinfo VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (message.guild.id, make_expiretime(int(sqlite3.connect("../DB/" + "license.db").cursor().execute("SELECT * FROM license WHERE code == ?;", (license_key,)).fetchone()[1])), "", "", first_pw, 0, "", "", 0, "", 0, 0, 0, 0, "파랑", 3, 5, 10, "익명님, `{product_name}` 제품 `{amount}`개 구매 감사합니다!", 0, 0, "", "기본 웹훅 프로필 url", "기본 웹훅 닉네임"))
                            con.commit()
        ...
        if message.content == '!버튼':
            if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
                if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                    ...
                    if color == "회색":
                        embed = discord.Embed(title=bot_name, description='원하시는 버튼을 클릭해주세요.', color=0x444444)
                        await message.channel.send(
                            embed=embed,
                            components = [
                                ActionRow(
                                    Button(style=ButtonStyle.gray,label = "제품",custom_id="제품"),
                                    Button(style=ButtonStyle.gray,label = "충전",custom_id="충전"),
                                    Button(style=ButtonStyle.gray,label = "정보",custom_id="정보"),
                                    Button(style=ButtonStyle.gray,label = "구매",custom_id="구매"),
                                )
                            ]
                        )
                        ...
                        if not int(user_info[3]) >= int(server_info[15]):
                            if not interaction.user.id in charginguser:
                                ...
                                try:
                                    ...
                                    res = requests.post("https://webvend.shop/api", json=jsondata)
                                except:
                                    try:
                                ...
                                if (res["result"] == True):
                                    ...
                                    try:
                                        ...
                                        if res["reason"] != "등록제한(10번 등록실패)" and res["reason"] != "잘못된 계정 정보":
                                            con = sqlite3.connect("../DB/" + str(interaction.guild.id) + ".db")
                                            cur = con.cursor()
                        ...
                        if money.isdigit():
                            if serverinfo[21] != "":
                                async def waiting():
                                    try:
                                        jsondata = {
                                            "api_key" : "", "bankpin" : server_info[21], "shop": interaction.guild.id, "userinfo" : name, "userid" : interaction.user.id, "token" : "token", "type" : True, "amount": int(money)
                                        }
                                        loop = asyncio.get_event_loop()
                                        bound = partial(
                                        requests.post, "", json=jsondata)
