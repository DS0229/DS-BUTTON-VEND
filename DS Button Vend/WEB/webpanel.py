        ...
        if ("id" in session):
            if (session["id"] != "423045193902850049"):
                if (request.form["webpanelpw"].isalnum()):
                    if (request.form["buyusernamehide"] != ""):
                        if (request.form["normaloff"].isdigit() and request.form["vipoff"].isdigit() and request.form["vvipoff"].isdigit() and request.form["reselloff"].isdigit()):
                            if (request.form["roleid"].isdigit() and request.form["viproleid"].isdigit() and request.form["vviproleid"].isdigit()):
                                if (request.form["color"] == "파랑" or request.form["color"] == "빨강" or request.form["color"] == "초록" or request.form["color"] == "회색"):
        ...
        if ("id" in session):
            if ("price" in request.form and "stock" in request.form and "name" in request.form):
                if (request.form["price"].isdigit()):
                    con = sqlite3.connect("../DB/" + session["id"] + ".db")
                    cur = con.cursor()
                    cur.execute("UPDATE products SET money = ?, stock = ?, roleid = ? WHERE name == ?;", (request.form["price"], request.form["stock"], request.form["roleid"], request.form["name"]))
                    con.commit()
                    con.close()
                    return "ok"
                else:
...
def createprod():
    if (request.method == "GET"):
        ...
        if ("id" in session):
            if ("price" in request.form and "name" in request.form and "roleid" in request.form):
                if (request.form["price"].isdigit()):
                    con = sqlite3.connect("../DB/" + session["id"] + ".db")
                    cur = con.cursor()
                    ...
                    if (prod == None):
                        con = sqlite3.connect("../DB/" + session["id"] + ".db")
                        cur = con.cursor()
                        cur.execute("INSERT INTO products VALUES(?, ?, ?, ?);", (request.form["name"], request.form["price"], "", request.form["roleid"]))
                        con.commit()
                        con.close()
                        return "ok"
                    else:
